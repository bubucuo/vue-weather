// mock data
// each call, return a different data, for temperature
export function fetchWeatherData(cityName) {
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve({
        ...weatherData,
        timezone: "America/" + cityName,
        current: {
          ...weatherData.current,
          temp: weatherData.current.temp + Math.random() * 5,
        },
        daily: weatherData.daily.map((day) => ({
          ...day,
          temp: {
            ...day.temp,
            max: day.temp.max + Math.random() * 5,
            min: day.temp.min + Math.random() * 5,
          },
        })),
        hourly: weatherData.hourly.map((hour) => ({
          ...hour,
          temp: hour.temp + Math.random() * 5,
        })),
      });
    }, 500);
  });
